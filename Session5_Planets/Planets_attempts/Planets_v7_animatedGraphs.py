...
class SolarSystem():
    ...
    def add_planet(self, planet):
        ...
        # self.fig.add_artist(momentum_line)
        # kinetic_energy_line, = self.ax_energy.plot([], [], lw=2, color='r')
        # self.fig.add_artist(kinetic_energy_line)
        # potential_energy_line, = self.ax_energy.plot([], [], lw=2, color='b')
        # self.fig.add_artist(potential_energy_line)
...
class Planet():
    ...
    def draw(self):
        ...
        self.SolarSys.ax_simulation.plot(
            *self.position,
            ...
            markersize=10,
            color=self.color,
            label=f"""L_{self.num} = {round(self.angular_momentum)} \nKE_{self.num} = {round(self.kinetic_energy)} \nGPE_{self.num} = {round(self.potential_energy)}"""
        )
    ...
    def draw_quantities(self):
        ...
        self.SolarSys.ax_momentum.plot(time_data, self.angular_momentums, color = self.color, label=f"Planet {self.num}")
        # ^ which can be seen to be constant over time (at least the sum of momenta from both planets is constant) 
        self.SolarSys.ax_energy.plot(time_data, self.kinetic_energies, color = self.color, label=f"Planet {self.num}")
        self.SolarSys.ax_energy.plot(time_data, self.potential_energies, color = self.color, linestyle="dashed", label=f"Planet {self.num}")
        self.SolarSys.ax_energy.plot(time_data, (np.array(self.potential_energies)+np.array(self.kinetic_energies)), color = self.color, linestyle="dotted", label=f"Planet {self.num}")
        # NOTE: in the energy plots, the solid line is for KEs, dashed for GPEs, and dotted for sum of energy. Dotted line approx constant, errors due to finite time issues
...
anim.save("Planets_animation_finalPlots_.gif", writer="pillow", dpi=200)
